...
import { BLOCK_CATEGORIES, BLOCK_DEFINITIONS, PICKABLE_BLOCK_TYPES, type BlockCategory } from "@/lib/site-builder/blockRegistry";
import type { BlockType } from "@/lib/site-builder/types";
...
export function BlockPickerPanel({ onPick, onClose }: BlockPickerPanelProps) {
  const [query, setQuery] = useState("");

  const grouped = useMemo(() => {
    const q = query.trim().toLowerCase();
    const filtered = q ? PICKABLE_BLOCK_TYPES.filter(({ type }) => BLOCK_DEFINITIONS[type].label.toLowerCase().includes(q)) : PICKABLE_BLOCK_TYPES;

    const byCategory = new Map<BlockCategory, BlockType[]>();
  ...
  return (
    <div className="fixed inset-y-0 right-0 z-40 w-96 border-l border-border bg-white shadow-2xl flex flex-col">
      <div className="flex items-center justify-between border-b border-border px-4 py-3">
        <h3 className="text-text-primary text-sm font-semibold">Add a Block</h3>
      ...
      <div className="flex-1 overflow-y-auto p-2">
        {grouped.size === 0 && <p className="text-text-muted p-4 text-center text-sm">No matching blocks.</p>}
        {BLOCK_CATEGORIES.filter((cat) => grouped.has(cat)).map((category) => (
