...
const alertVariants = cva(
  "relative w-full rounded-card border p-4 text-sm [&>svg]:absolute [&>svg]:left-4 [&>svg]:top-4 [&>svg]:size-4 [&>svg+div]:translate-y-[-3px] [&:has(svg)]:pl-11",
  {
    variants: {
      variant: {
        default: "border-border bg-white text-text-primary",
        destructive: "border-danger/50 bg-red-50 text-danger [&>svg]:text-danger",
      },
    },
